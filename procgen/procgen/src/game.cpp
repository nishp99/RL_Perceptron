...
void Game::parse_options(std::string name, VecOptions opts) {
    ...
    opts.consume_int("agent_health", &options.agent_health); //CHANGED
    opts.consume_int("tmax", &options.tmax); //CHANGED
...
void Game::serialize(WriteBuffer *b) {
    ...
    //CHANGED
    b->write_int(options.agent_health);
    b->write_int(options.tmax);
...
void Game::deserialize(ReadBuffer *b) {
    ...
    //CHANGED
    options.agent_health = b->read_int();
    options.tmax = b->read_int();
