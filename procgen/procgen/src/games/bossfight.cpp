...
class BossfightGame : public BasicAbstractGame {
  public:
    ...
    int agent_health = 1; //CHANGED
    int tmax = 200; //CHANGED
    ...
    BossfightGame()
        ...
        agent_health = options.agent_health; //CHANGED
        tmax = options.tmax; //CHANGED
    }
    ...
    void handle_agent_collision(const std::shared_ptr<Entity> &obj) override {
        ...
        if (obj->type == BOSS) {
            ...
            //step_data.done = true;
            agent_health -= 1; //CHANGED
            step_data.reward -= 1; //CHANGED
        }
        if (obj->type == ENEMY_BULLET) {
            //step_data.done = true;
            obj->will_erase = true; // Erase projectile CHANGED
            agent_health -= 1; // Subtract a life CHANGED
            step_data.reward -= 1; // CHANGED
            //if (agent_health == 0)
              //  step_data.done = true;
    ...
    void game_reset() override {
        ...
        options.center_agent = true; //CHANGED
        ...
        //spawn_barriers(); //CHANGED
        spawn_edge_barriers();  //CHANGED
    ...
    void game_step() override {
        ...
        if (cur_time > tmax) { // Stop episode at deadline CHANGED
            step_data.done = true;
        }
    }
    ...
    void serialize(WriteBuffer *b) override {
        ...
        b->write_int(agent_health); //CHANGED
        b->write_int(tmax); //CHANGED
    ...
    void deserialize(ReadBuffer *b) override {
        ...
        agent_health = b->read_int(); //CHANGED
        tmax = b->read_int(); //CHANGED
